...
標高の取得は非同期で行うため、取得中にユーザーが「1つ戻す」「リセット」やクリックを行うと、
削除した区間や以前のルートの結果が後から反映されるおそれがある。これを防ぐため、以下のように制御する。

- AppControllerは、ルートが変更される(地点の追加・削除・リセット)たびに**世代番号**を1増やす
- 標高の取得を開始するときに、その時点の世代番号を記録しておく
- 取得完了時に、記録した世代番号が現在の世代番号と異なる場合は、その結果をルートに反映せず破棄する
  (取得したタイルはキャッシュに残るため、再取得の無駄は生じない)
- 取得中も地図のクリック・ボタン操作は無効化しない
- 取得中は、断面図エリアに取得中であることを表示する
